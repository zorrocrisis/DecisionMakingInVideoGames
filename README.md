...
- There are actions whose effects are ambiguous while only considering four goals and one action at a time. Thus, there is a need for the programmer to explicitly “show” to the character that a given action is indirectly good and will decrease the discontentment in the future. As an example, we can think about how GetManaPotion influences the goal values (commented on code) …
...
- Sometimes Sir Uthgard does not care for his own short-term survivability... For instance, he may decide the best sequence of actions is to SwordAttack an Orc, SwordAttack the Dragon (which will most likely get him killed) and UseHeathPotion, ignoring whether he will live after the second action just because the final discontentment is better. We could improve this behaviour by checking if the character hasn’t died after each hypothetical action, although this could lead to a decrease in performance!

- Other similarly odd behaviours are rooted in the same cause – by choosing a sequence of actions based only on the future discontentment, the main character risks performing actions that hinder him or even prevent him from completing “the current smartest move”.

This aspect of GOAP can be mitigated (but not eliminated) by increasing the action combinations processed per frame, hopefully reaching better discontentment values and a sequence of action that doesn’t risk the short-term livelihood of Sir Uthgard. This change, however, comes at a performance cost which should be noted.
...
- By performing multiple playouts (introducing a stochastic environment), the number of nodes with a higher/positive win ratio also increases. This version of MCTS was implement in code – MCTS Multiple Playouts – and by activating the MCTS Active field in the character’s inspector window, we can even choose the Number of Playouts done (one playout corresponds to the vanilla MCTS, of course). One of the major differences from the original algorithm is the time processed during the decision making: using 5 playouts, the character went from deciding the next best action in just 40 milliseconds to taking around 150 milliseconds (almost 4 times as long)! Once again, this approach can hinder the overall performance of the algorithm.

- Variations of the vanilla MCTS, such as MCTS with biased playouts and limited playouts can lead to better results. Both options - MCTSBiasedPlayout, MCTSLimitedPlayout - were implemented as children of the MCTS class since only the Playout method is different. These algorithms can be accessed through the character’s inspector window, while the MCTS Active field is active.

Regarding the MCTS w/Biased Playouts, heuristic information related to each action was added to help the playout phase choose estimated best actions. As an example, health potions were made more relevant late game (when Sir Uthgard has a higher level and can therefore heal more HP points with a single potion), while praying had an emphasis early on... The main character is also discouraged from performing attacks (namely sword attacks) that will get him killed, significantly boosting the win ratios of each node. An effort was made to always choose different heuristic values so there wouldn't be a lot of ties between actions, in which case we would come full circle and have random playouts. It’s also worth stating that our implementation of the biased playout follows the greedy approach, meaning that the algorithm will always choose the action with the lowest heuristic. Alternatively, we could've opted for the Gibbs/Softmax approach, although that would increase the complexity and duration of the playouts. The performance of the algorithm didn’t decrease significantly: the character went from deciding the next best action in just 40 milliseconds to taking around 100 milliseconds (time processed increased 2.5 times).

Lastly, the MCST w/Limited Playouts aimed to increase the efficiency of MCTS by limiting the depth of playouts and using a heuristic function to estimate the quality of the world state reached, which replaced the usual reward value. The heuristic function was implemented in the FutureStateWorldModel class and takes into account important stats of the main character in the simulated world state (health points, level and money) which hint to a possible victory or simply a favourable position to be in. This function consists of a weighted sum of these stats. The resulting heuristic value is normalized and finally returned.
