...
To test the application, only the files contained in the "Build" folder are necessary. The application can also be found on [this separate repository](https://github.com/zorrocrisis/DecisionMakingInVideoGamesBuildOnly). Once the build files are downloaded, simply start the executable (see the controls below).
